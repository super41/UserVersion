...
public class WifiConnectActivity extends AppCompatActivity implements SocketUtil.SocketImp {
    ...
    void sendFirst(){
        String s=pg.getQrCode() + pg.getEmail();
        socketUtil.send(s,0x01);
        byte[] b=SocketUtil.getByte(s,0x01);
        tv_send.append("邮箱: "+byteToString(b)+"\n");
    }
   ...
   void send() {
    ...
    public void onResult(byte[] b) {
        ...
        switch(STEP){
            case 1: if(checkReply1(b)){
                    STEP=2;
                    sendSecond();
                }else{
                Toast.makeText(WifiConnectActivity.this,"邮箱检测不过",Toast.LENGTH_SHORT).show();
            }
                    break;

            case 2: if(checkReply2(b)){
                  STEP=3;
                  sendThree();
            }else{
                Toast.makeText(WifiConnectActivity.this,"wifi名称检测不过",Toast.LENGTH_SHORT).show();
            }
                  break;
            case 3: if(checkReply3(b)){
                STEP=4;
                sendFour();
            }else{
                Toast.makeText(WifiConnectActivity.this,"wifi密码检测不过",Toast.LENGTH_SHORT).show();
            }
                break;
            case 4: if(checkReply4(b)){
                STEP=5;
                sendFive();
            }else{
                Toast.makeText(WifiConnectActivity.this,"距离检测不过",Toast.LENGTH_SHORT).show();
            }
                break;
            case 5: if(checkReply5(b)){
                sendSix();
                STEP=6;
            }else{
                Toast.makeText(WifiConnectActivity.this,"邮件标题检测不过",Toast.LENGTH_SHORT).show();
            }
                break;
            case 6:if(checkReply6(b)){
                ...
                Toast.makeText(WifiConnectActivity.this,"邮件正文不过",Toast.LENGTH_SHORT).show();
            }
                STEP=1;
                break;
        }
    ...
    boolean checkReply1(byte[] b){
        String s=pg.getQrCode();
        byte[] rightByte=getTrueReplyByte(s,0x01);
